...
func CreateEventType(db *sql.DB, et *EventType) (int64, error) {
	res, err := db.Exec(`
		INSERT INTO event_types
			(user_id, slug, title, description, confirmed_message, rejected_message, duration_minutes, color, booking_window_days)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, et.UserID, et.Slug, et.Title, et.Description, et.ConfirmedMessage, et.RejectedMessage,
...
func ListEventTypes(db *sql.DB, userID int64) ([]EventType, error) {
	...
	for rows.Next() {
		var et EventType
		if err := rows.Scan(&et.ID, &et.UserID, &et.Slug, &et.Title, &et.Description, &et.ConfirmedMessage, &et.RejectedMessage,
...
func GetEventType(db *sql.DB, id, userID int64) (*EventType, error) {
	...
	`, id, userID).Scan(&et.ID, &et.UserID, &et.Slug, &et.Title, &et.Description, &et.ConfirmedMessage, &et.RejectedMessage,
...
func GetEventTypeBySlug(db *sql.DB, userID int64, slug string) (*EventType, error) {
	...
	`, userID, slug).Scan(&et.ID, &et.UserID, &et.Slug, &et.Title, &et.Description, &et.ConfirmedMessage, &et.RejectedMessage,
		&et.DurationMinutes, &et.Color, &et.BookingWindowDays, &et.Active, &et.CreatedAt)
	if err != nil {
		return nil, err
	}
...
func UpdateEventType(db *sql.DB, et *EventType) error {
	_, err := db.Exec(`
		UPDATE event_types SET
			title = ?, description = ?, confirmed_message = ?, rejected_message = ?, duration_minutes = ?,
			color = ?, booking_window_days = ?
		WHERE id = ? AND user_id = ?
	`, et.Title, et.Description, et.ConfirmedMessage, et.RejectedMessage, et.DurationMinutes,
