...
func TestCalculateSlots_Basic(t *testing.T) {
	...
	slots := CalculateSlots(rules, nil, nil, date, 30*time.Minute, loc, 60, now)
...
func TestCalculateSlots_EventBlocking(t *testing.T) {
	...
	slots := CalculateSlots(rules, events, nil, date, 30*time.Minute, loc, 60, now)
...
func TestCalculateSlots_EventBlockingCrossTimezone(t *testing.T) {
	...
	now := time.Date(2026, 4, 28, 0, 0, 0, 0, berlin)
	...
	slots := CalculateSlots(rules, events, nil, date, 30*time.Minute, berlin, 60, now)
...
func TestCalculateSlots_WrongWeekday(t *testing.T) {
	...
	now := time.Date(2026, 4, 28, 0, 0, 0, 0, loc)
